...
var GLSLStyle = function(){
    ...
    function loadGlslElements() {
        ...
        // parse EDITORS
        ccList = document.querySelectorAll(".codeAndCanvas");
        for(var i = 0; i < ccList.length; i++){
            if (ccList[i].hasAttribute("data")){
                var srcFile = ccList[i].getAttribute("data");
                newCodeEditor = new GlslEditor(ccList[i], { 
                    canvas_size: 250, 
                    canvas_follow: true,
                    canvas_float: 'right',
                    tooltips: true, 
                ...
                newCodeEditor.open(srcFile);
                glslEditors.push(newCodeEditor);
            }
        }

        // parse GRAPHS
        sgList = document.querySelectorAll(".simpleGraph");
        for(var i = 0; i < sgList.length; i++){
            if (sgList[i].hasAttribute("data")){
                var srcFile = sgList[i].getAttribute("data");

                // sgList[i].id = 'simpleGraphContainer' + i
                newGraphEditor = new GlslEditor(sgList[i], { 
                    canvas_width: container.offsetWidth - 40, 
                    lineNumbers: true, 
                    canvas_follow: true,
                    canvas_float: false,
                ...
                glslGraphs.push(newGraphEditor)
