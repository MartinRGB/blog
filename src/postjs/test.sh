...
# Get Line Num
# sed -n '/router-link/=' ${listfile} | sed -e 's/<a>\(.*\)<a>/\1/' | sort -t '>' -n -k4 ${listfile} 
# IFS=$'\n' lineArray=($(sed -n '/router-link/=' ${listfile} | sed -e 's/<a>\(.*\)<a>/\1/'))
# echo ${lineArray[@]}

# Get KeyWord | Cut String after . | sort
# sed -e '/<a>/!d' -e 's/.*<a>\([^>]*\)<\/a><\/li><\/router-link>/\1/' ${listfile} | cut -d. -f1
# | sort -n 
...
awk '/\*Add List Here/{f=1;next}/\*Add List Finish/{f=0}f' ${listfile} | sort -t '>' -n -k4 > list3.vue
# firstSelect=$(awk '/Add List Here/,/Add List Finish/ {print}' < ${listfile})|sort -t '>' -n -k4 > list3.vue
# echo $firstSelect
